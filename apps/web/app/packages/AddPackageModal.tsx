...
export default function AddPackageModal() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setPlans([{ name: "", price: "", numberOfPersons: "2", features: "" }]);
      ...
      setTimeout(() => {
        setOpen(false);
        setSuccess(false);
        router.refresh(); // Re-fetch packages list
      }, 1200);
    } catch (err: any) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-[80vw] lg:max-w-5xl max-h-[92vh] overflow-y-auto bg-slate-950 text-white border border-slate-900 rounded-2xl p-6 shadow-2xl">
        <DialogHeader className="pb-2">
          <DialogTitle className="text-2xl font-bold tracking-tight text-white flex items-center gap-2">
            <span className="text-amber-500">✨</span> Add New Tour Package
          </DialogTitle>
        </DialogHeader>
        ...
        {success ? (
          <div className="flex flex-col items-center justify-center py-16 gap-4 text-center">
            ...
            <p className="text-sm text-slate-400">
              Refreshing your tour packages dashboard...
            </p>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-6 mt-2">
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              ...
              <TabsContent value="gallery" className="space-y-4 outline-none">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 max-h-[48vh] overflow-y-auto pr-1">
                  {gallery.map((url, idx) => (
                    <div key={idx} className="relative group aspect-video border border-slate-800 rounded-xl overflow-hidden bg-slate-900 shadow-inner">
                    ...
                    {itinerary.map((item, idx) => (
                      <div key={idx} className="relative border border-slate-900 bg-slate-900/40 hover:border-amber-500/20 transition-all rounded-xl p-4 flex flex-col gap-3.5 shadow-inner">
                        <div className="flex justify-between items-center">
                          <Badge className="bg-amber-500 text-black font-bold text-xs py-1 px-3 rounded-md animate-fade-in">
                            Day {item.day}
                          </Badge>
                          <Button
                            type="button"
            ...
            {error && (
              <div className="text-xs text-red-400 bg-red-950/40 border border-red-900/30 rounded-xl px-4 py-3 flex items-start gap-2 animate-shake">
